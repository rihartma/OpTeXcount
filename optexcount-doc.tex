...
It counts words and other elements of \OpTeX/ document and sorts them into individual categories. These categories are inspired by \TeX count.
...
Regular text words are words that occur in the main text. Header words are introduced using these particular keywords: {\Blue `\tit`}, {\Blue `\chap`}, {\Blue `\sec`} and {\Blue `\secc`}. Caption words takes into account captions of tables and figures, i.e., {\Blue `\caption/t`} and {\Blue `\caption/f`}, footnotes({\Blue `\fnote`} or {\Blue `\fnotetext`}) and marginal notes({\Blue `\mnote`}). Header count is the number of all headers in our document. Figures/Tables count counts all tables and figures which occur in the document. Inline formulae are introduced using {\Blue `$`\dots`$`} and displayed formulae are introduced using {\Blue `$$`\dots`$$`}.
...
This get this utility running, Python3 has to be installed. Then user can run this utility by command:

\begtt
python3 optexcount.py optex-filename
\endtt

User can also use {\Blue `-verbose`} specifier. The source code will be printed on the screen with colored words, so he can easily see, what is considered as text word, header word, caption word, keyword etc.
